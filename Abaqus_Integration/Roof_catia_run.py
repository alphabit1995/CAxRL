...
def CreateRoof(positions, breadth):
    ...
    p = mdb.models['Model-1'].Part(name='Roof', dimensionality=THREE_D, 
        type=DEFORMABLE_BODY)
    p = mdb.models['Model-1'].parts['Roof']
...
def Create_Sections():
    mdb.models['Model-1'].HomogeneousShellSection(name='Roof_Section', 
...
def View_Results():
    ...
    session.viewports['Viewport: 1'].setValues(
        displayedObject=session.odbs['Job-1.odb'])
...
for i in range(len(odb.steps['Step-1'].frames[-2].fieldOutputs['S'].values)):
    total_stress += odb.steps['Step-1'].frames[-1].fieldOutputs['S'].values[i].mises

average_stress = total_stress/len(odb.steps['Step-1'].frames[-1].fieldOutputs['S'].values)

print("Average Stress of the system: ", average_stress)

odb.close()
os.remove("Job-1.odb")

print >> sys.__stdout__, str(average_stress)
